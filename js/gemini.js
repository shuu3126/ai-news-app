...
async function _expandNarration(narration, summarizedNews, apiKey, model, signal) {
  ...
  const expandPrompt = `
以下は本日のAI活用ニュースのナレーション原稿ですが、目標の5000文字に対して${currentLength}文字と短すぎます。

同じ構成・同じ話し言葉（です・ます調）のまま、各トピックの「どのツールをどう使ったか」「どんな成果が出たか」「自分の仕事にどう応用できるか」の解説を厚くして、全体で5000文字以上6000文字程度になるよう書き直してください。

- 冒頭は『おはようございます、本日のAI活用ニュースをお伝えします』のまま、末尾は『以上、本日のAI活用ダイジェストでした』のままにしてください。
...
以下は本日のAI活用ニュースのナレーション原稿ですが、${currentLength}文字あり、目標の15分（5000〜6000文字）に対して長すぎます。

同じ構成・同じ話し言葉（です・ます調）のまま、全体で5000文字以上6000文字以内に収まるよう書き直してください。

- 実務に活かしにくいトピックは丸ごと削ってください。全部を残そうとして各トピックを薄くするより、件数を減らして残したトピックの解説を保つほうが良いです。
- 実際の導入事例・具体的な活用ノウハウ・すぐ真似できる自動化の話は最優先で残してください。逆に、モデルの技術的詳細やベンチマーク、資金調達の話は真っ先に削ってください。
- 冒頭は『おはようございます、本日のAI活用ニュースをお伝えします』のまま、末尾は『以上、本日のAI活用ダイジェストでした』のままにしてください。
- 記号やMarkdown装飾は使わないでください。
...
あなたは中小企業の副業経営者向けにAI活用情報を発信する専門家です。以下の情報源を中心に、直近24〜72時間以内の記事・投稿から、次の条件に当てはまるものを検索して要約してください。

優先して拾ってほしい内容（この基準で選ぶこと）:
- 実際に企業・個人事業主がAIを導入して売上・業務効率・コスト削減などの具体的な成果を出した事例
- 「このツール・スクリプトをこう使うと便利」という具体的な活用方法・自動化のやり方
- 画期的・独創的な新しいAIツール・スクリプト・サービスで、副業（Web制作・VTuber事務所運営等）に応用できそうなもの
- Google（Gemini・NotebookLM・Google Workspace・Google Cloud AI等）の最新機能と、他のツールを組み合わせて業務を自動化する具体例
- 一般ユーザー・中小事業者がすぐ真似できる、再現性のあるノウハウ

優先度を下げてよい内容（できるだけ避ける）:
- モデルの内部アーキテクチャ・ベンチマークスコア・研究論文などの技術的詳細
- 資金調達額・企業間の競争・株価など経営ニュースそのもの（活用事例と無関係なもの）
- AI安全性・規制・政策に関する議論（よほど実務に直結しない限り）

各トピックについて、「何を」「どう使ったら」「どんな成果が出たか／どう便利になるか」が分かるように、具体性を持って要約してください。

後続の日本語ナレーション生成で5000〜6000文字（読み上げて約15分）の原稿を作ります。それにちょうど見合う分量にしてください。以下を必ず守ってください。
- 拾うトピックは全体で10件から12件に絞ってください。多すぎても困ります。基準に合わないものは落としてください。
- 各トピックは3〜4文で書いてください。何が起きたかだけでなく、具体的な手順・使ったツール・出た数字（売上、削減できた時間、コスト等）まで含めてください。単なる見出しの羅列にしないでください。
- 製品名・機能名・具体的な数字を省略せずに書いてください。
- 同じ内容の言い換えによる水増しはしないでください。分量は情報量で満たしてください。
- 各トピックの先頭に情報源名を書いてください。URLは含めないでください。

情報源（優先度の参考。他に良い情報源があれば自由に検索して構わない）:
- anthropic.com/customers（Claude導入企業の事例・成果）
- openai.com/stories, openai.com/business（OpenAI導入企業の事例）
- Google Workspace・Google Cloud のAI活用ブログ（Gemini・NotebookLM・Google AI連携の新機能や自動化事例）
- TLDR AI, The Rundown AI, The Neuron（実務者向けAI活用ニュースレター）
- note.com・Zenn・Qiita 等の日本語コミュニティで話題のAI活用・自動化スクリプト記事
- Product Hunt の直近のAI系新着プロダクト（実用ツール・自動化サービス）
...
あなたは副業経営者向けのAI活用ラジオ番組のナレーターです。以下のニュース要約を元に、15分前後で読み上げられる自然な話し言葉の日本語ナレーション原稿を作成してください。

分量の指定（最優先で守ってください）:
- 全体で5000文字以上6000文字以内にしてください。6000文字を超えてはいけません。
- 取り上げるトピックは10件程度に絞ってください。1トピックあたり400文字から500文字が目安です。
- 下のニュース要約に載っているトピックを全部詰め込む必要はありません。実務に活かしにくいものは思い切って落としてください。網羅性より15分という尺を優先してください。

その他の制約:
- 敬体（です・ます調）を使用してください。
- 記号やMarkdown装飾（#、*、|、箇条書きのハイフンなど）は一切使用しないでください。地の文だけで書いてください。
- URLは読み上げさせず、『詳細はニュースレターを参照』などの表現に留めてください。
- 各トピックについて、何が起きたかだけでなく、どのツールをどう使ったのか、どんな成果（売上、削減できた時間、コスト）が出たのかという具体的なところまで解説してください。
- 単なる出来事の紹介で終わらせず、『これは副業のWeb制作でこう応用できそうです』のように、聞き手が自分の仕事に置き換えて考えられるひとことを、可能なトピックには添えてください（無理にこじつけなくてよい）。
- モデルの内部構造やベンチマークスコアの話には踏み込まないでください。聞き手は技術的な詳細に関心がありません。
- 同じ内容を言い換えて繰り返す水増しはしないでください。情報量で分量を満たしてください。
- 英語の製品名や略語は、初出時にカタカナ読みを添えるなど、耳で聞いて分かる表現にしてください。
- 音声で読み上げるため、一文は長くなりすぎないようにしてください。

構成:
- 『おはようございます、本日のAI活用ニュースをお伝えします』で始めてください。
- 実際の導入事例・成果が出た話を最優先で、最初に詳しく伝えてください。
- 次に、便利な使い方・自動化の具体的なノウハウを伝えてください。
- 最後に、Googleの最新AI機能や新しいツール・スクリプトの紹介を伝えてください。
- 各情報源については、『〜によりますと』『〜が伝えています』のように話し言葉で出典に触れてください。
- 話題が変わるところでは『続いてのニュースです』のような繋ぎの一文を入れてください。
- 『以上、本日のAI活用ダイジェストでした』で締めてください。
